...
class PreferenciaPersonal(models.Model):
    #id= models.AutoField(primary_key=True)
    nombrepreferenciapersonal = models.CharField(max_length=30,primary_key=True)
...
class UsuarioProductoEmpresaTitular(models.Model):
    id = models.AutoField(primary_key=True)
    usuario= models.ForeignKey(Usuario,to_field='username',on_delete=models.CASCADE)
    idproductoempresatitular = models.ForeignKey(ProductoEmpresaTitular,to_field='id',on_delete=models.CASCADE)
    def __str__(self):
        return '{}'.format(self.usuariio)
...
class UsuarioPreferencia(models.Model):
    id = models.AutoField(primary_key=True)
    usuario = models.ForeignKey(Usuario,to_field='username',on_delete=models.CASCADE)
    preferencia = models.ForeignKey(PreferenciaPersonal,to_field='nombrepreferenciapersonal',on_delete=models.CASCADE)
    def __str__(self):
